        ...
        return this._http.post("https://api.privatbank.ua/p24api/rest_fiz", body)
            .map((response: Response) => {
                ...
                xml2js.parseString(response.text(), function (err, result) {
                    ...
                    res.idMerchant = result.response.merchant[0].id[0] as number;
                    res.credit = result.response.data[0].info[0].statements[0].$.credit as string;
                    res.debet = result.response.data[0].info[0].statements[0].$.debet as string;
                    for (let i = 0; i < result.response.data[0].info[0].statements[0].statement.length; ++i) {
                        ...
                        temp.trandate = result.response.data[0].info[0].statements[0].statement[i].$.trandate as string;
                        temp.trantime = result.response.data[0].info[0].statements[0].statement[i].$.trantime as string;
